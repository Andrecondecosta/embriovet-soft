...
- Módulo **Registrar Inseminação** redesenhado (UX técnica, sem mudar lógica de negócio central):
  ...
  - Nova página **Importar Sémen**: template XLSX/CSV (com fallback se openpyxl não estiver disponível), upload, preview, validação (incluindo qualidade 0-100) com tabela de erros, importação transacional e relatório de importação
